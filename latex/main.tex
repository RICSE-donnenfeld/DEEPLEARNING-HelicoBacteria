...
Training on a GPU cluster is launched via \texttt{train\_classifier.sbatch.sh} and \texttt{train\_autoencoder.sbatch.sh} (legacy \texttt{train.sbatch.sh} is kept as a compatibility script). Dataset paths follow the \texttt{HelicoDataSet/} layout (CrossValidation/Annotated, HoldOut). Diagnostics and CV interpretation scripts (\texttt{analyze\_cv\_metrics.py}, \texttt{analyze\_ae\_fold\_diagnostics.py}) produce plots and robust-statistics summaries under \texttt{cv/plots/}. Reproducibility is ensured by fixed random seeds, patient-level splitting, and per-fold metric/checkpoint logging.
