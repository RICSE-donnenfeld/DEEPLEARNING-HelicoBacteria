...
HoldOut-per-fold (mean$\pm$std) & CNN + aggregation & 0.8172$\pm$0.0065 & 0.8439$\pm$0.0173 & 0.7793$\pm$0.0129 & 0.8552$\pm$0.0207 & 0.8101$\pm$0.0053 \\
HoldOut-per-fold (mean$\pm$std) & AE + aggregation & 0.6966$\pm$0.0675 & 0.6860$\pm$0.0750 & 0.7586$\pm$0.0218 & 0.6345$\pm$0.1516 & 0.7172$\pm$0.0383 \\
\bottomrule
\end{tabular}
...
The main observed limitation is threshold instability under distribution shift: fold 3 presents an upward shift of reconstruction errors for both classes, which inflates local threshold and degrades specificity at patient level. A pooled global threshold improved some folds but harmed specificity severely on fold 3, so a single global threshold is not yet robust enough. The current fold-consistent patient pipeline makes this behavior explicit and reproducible, and provides a solid baseline for future threshold normalization/calibration strategies (e.g., robust scaling or nested calibration).

% ==========================================================
\section{Conclusion}
\label{sec:conclusion}
We implemented two patch-level models for H.\textit{ pylori} detection: a supervised CNN and an unsupervised autoencoder for anomaly detection, and evaluated both with patient-level 5-fold CV plus fold-consistent patient-level aggregation.
...
The repository now includes reproducible CV diagnostics, fold-consistent patient-level evaluation, and report-generation tooling. The main remaining improvement track is threshold calibration for AE to reduce false positives while preserving sensitivity.
